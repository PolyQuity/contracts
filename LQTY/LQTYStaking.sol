...
contract LQTYStaking is ILQTYStaking, Ownable, CheckContract, BaseMath {
    ...
    uint public F_LQTY; // Running sum of LQTY fees per-LQTY-staked
    ...
    event F_LQTYUpdated(uint F_LQTY);
    ...
    function stake(uint _LQTYamount) external override {
        ...
        emit StakingGainsWithdrawn(msg.sender, LUSDGain, ETHGain, LQTYGain);

         // Send accumulated LUSD、LQTY and ETH gains to the caller
        if (currentStake != 0) {
    ...
    function increaseF_LQTY(uint _LQTYFee) external override {
        _requireCallerIsLQTYToken();
        uint LQTYFeePerLQTYStaked;
        
        if (totalLQTYStaked > 0) {LQTYFeePerLQTYStaked = _LQTYFee.mul(DECIMAL_PRECISION).div(totalLQTYStaked);}
        
        F_LQTY = F_LQTY.add(LQTYFeePerLQTYStaked);
        emit F_LQTYUpdated(F_LQTY);
    }
    ...
    function getPendingLQTYGain(address _user) external view override returns (uint) {
    ...
    function _getPendingLQTYGain(address _user) internal view returns (uint) {
        ...
        uint LQTYGain = stakes[_user].mul(F_LQTY.sub(F_LQTY_Snapshot)).div(DECIMAL_PRECISION);
        return LQTYGain;
    }
    ...
    function _updateUserSnapshots(address _user) internal {
        ...
        emit StakerSnapshotsUpdated(_user, F_ETH, F_LUSD, F_LQTY);
    }
    ...
    function _requireCallerIsLQTYToken() internal view {
        require(msg.sender == address(lqtyToken), "LQTYStaking: caller is not lqtyToken");
    }
